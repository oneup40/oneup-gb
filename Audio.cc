...
void Audio::GenerateSample() {
    //std::cerr << "GenerateSample" << std::endl;
    //u8 so1 = (nr50_ & 0x07) << 4,
    //   so2 =  nr50_ & 0x70;
    ...
    if (nr52_ & 0x01) {
        //std::cerr << "\tch1 enabled" << std::endl;
        ch1_.TickOutput();

        if (nr51_ & 0x10) { /* std::cerr << "\tch1 -> so1" << std::endl; */ so1 += ch1_.vout_; }
        if (nr51_ & 0x01) { /* std::cerr << "\tch1 -> so2" << std::endl; */ so2 += ch1_.vout_; }
    }
